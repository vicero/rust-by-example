...
fn rect_area(rectangle: Rectangle) -> f32 {
    let Rectangle { top_left: Point { x, y: _ }, bottom_right: Point { x: _, y }} = rectangle;
    x * y
}

fn square(top_left: Point, size: f32) -> Rectangle {
    let Point { x: x0, y: y0 } = top_left;
    let bottom_right = Point { x: x0 + size, y: y0 + size };
